...
export async function runAnalysis(team1Name, team2Name, proxyUrl, options = {}) {
  ...
  if (oddsApiKey) {
    ...
    oddsApiResult = await searchOddsApiMatch(en1, en2, oddsApiKey);
    if (!oddsApiResult) {
      oddsApiResult = await searchOddsApiMatch(team1Name, team2Name, oddsApiKey);
    }
  }
  ...
  if (nationalTeams) {
    ...
    const elo1X2 = eloTo1X2(eloBase1, eloBase2);
    const probs = elo1X2;
