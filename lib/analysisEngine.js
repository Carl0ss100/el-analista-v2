...
export function fifaRankToElo(rank) {
  if (rank <= 1) return 2100;
  if (rank <= 3) return 2100 - (rank - 1) * 15;
  if (rank <= 5) return 2070 - (rank - 3) * 10;
  if (rank <= 10) return 2050 - (rank - 5) * 8;
  if (rank <= 20) return 2010 - (rank - 10) * 6;
  if (rank <= 40) return 1950 - (rank - 20) * 4;
  if (rank <= 80) return 1870 - (rank - 40) * 3;
  return Math.max(1500, 1750 - (rank - 80) * 2);
}
...
export async function runAnalysis(team1Name, team2Name, proxyUrl, options = {}) {
  ...
  if (nationalTeams) {
    ...
    const isNeutralVenue = !competition || /world cup|copa del mundo|mundial|euro|nations league|copa america|amistoso|friendly/i.test(competition);
    const isQualifier = competition && /qualif|eliminat/i.test(competition);
    const homeAdv = isQualifier ? 65 : 0;

    const eloBase1 = t1.fifaRank ? fifaRankToElo(t1.fifaRank) : 1500;
    const eloBase2 = t2.fifaRank ? fifaRankToElo(t2.fifaRank) : 1500;
    const elo1X2 = eloTo1X2(eloBase1, eloBase2, homeAdv);

    const [homeForm, awayForm] = await Promise.all([
      fetchForm(t1.api_id, proxyUrl).catch(() => []),
      fetchForm(t2.api_id, proxyUrl).catch(() => []),
    ]);

    const allFixtures = [...homeForm, ...awayForm].filter(f => f.fixtureId);
    const { fixtureId } = await findFixtureId(t1.api_id, t2.api_id, proxyUrl, dateStr, allFixtures);
    ...
    if (fixtureId) {
      ...
      h2hData = extraResults[0];
      injuries1 = extraResults[1];
      injuries2 = extraResults[2];
      lineups = extraResults[3];
    } else {
      h2hData = await fetchH2H(t1.api_id, t2.api_id, proxyUrl).catch(() => null);
    }
    ...
    if (fixtureId) {
      try {
        apiFootballOdds = await fetchOdds(fixtureId, proxyUrl).catch(() => null);
      } catch {}
    }
    ...
    return {
      ...
      isNeutralVenue,
      homeAdv,
      confederations: { team1: t1.confederation, team2: t2.confederation },
      commence: oddsApiResult?.commence || null,
    };
  }
