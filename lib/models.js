...
export function eloExpected(eloHome, eloAway, homeAdvantage = ELO_HOME_ADVANTAGE) {
...
export function eloTo1X2(eloHome, eloAway, homeAdvantage = ELO_HOME_ADVANTAGE) {
  const eHome = eloExpected(eloHome, eloAway, homeAdvantage).home;
  const eAway = 1 - eHome;
  const drawLine = 0.26;
  const drawChance = drawLine * (1 - Math.abs(2 * eHome - 1));
  const homeChance = eHome * (1 - drawChance);
  const awayChance = eAway * (1 - drawChance);
